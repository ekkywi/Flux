...
@section("content")
                            ...
                            <tr class="hover:bg-slate-50/30 transition-colors group">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex flex-col gap-1">
                                        ...
                                        <span class="text-[8px] font-black uppercase tracking-[0.15em] text-indigo-500/60">Tier 1 Access</span>
                                    </div>
                                </td>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <button class="p-2 text-slate-400 hover:text-indigo-600 transition-colors rounded-lg hover:bg-indigo-50" title="Edit Entity">
                                        ...
                                        <button class="p-2 text-slate-400 hover:text-rose-600 transition-colors rounded-lg hover:bg-rose-50" title="Revoke Access">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="fixed inset-0 z-[100] items-center justify-center hidden bg-slate-900/60 backdrop-blur-sm px-4" id="provisionModal">
        <div class="bg-white w-full max-w-md rounded-[2.5rem] p-10 shadow-2xl border border-slate-200 transform transition-all">
            ...
            <form action="{{ route("admin.users.store") }}" class="space-y-5" method="POST">
                ...
                <div class="space-y-1.5">
                    <label class="text-[9px] font-black uppercase text-slate-400 ml-1">Security Role</label>
                    <select class="w-full px-5 py-3.5 rounded-2xl border-slate-200 text-sm focus:ring-indigo-500 focus:border-indigo-500 bg-slate-50 appearance-none font-bold text-slate-700" name="role" required>
                        <option disabled selected value="">Assign Security Tier</option>
    ...
    @push("scripts")
        <script>
            ...
            @if ($errors->any())
                toggleProvisionModal();
            @endif

            setTimeout(() => {
                const toasts = document.querySelectorAll('.animate-bounce');
